...
# Agrupador de paradas (colapsa blocos contínuos da mesma operação)
...
app.layout = html.Div(style={"backgroundColor": "#f8f9fa", "padding": "20px"}, children=[
    dbc.Container([
        ...
        # Filtros principais (como já existia)
        ...
        # Cards de stats + gráfico (como já existia)
        ...
        # >>> NOVO: Resumo diário por operador (uma única aba/área na mesma página)
        dbc.Card(dbc.CardBody([
            html.H4("Resumo diário por operador (todos no dia selecionado)", className="mb-3"),
...
def atualizar_grafico(operador, equipamento, data_str):
    ...
    # Excluir 'FINAL DE EXPEDIENTE' para cálculo/visual
...
# ========= NOVO CALLBACK: Tabela "Resumo diário por operador" =========
@app.callback(
...
def atualizar_resumo_dia(data_str):
    ...
    for nome, grupo_raw in df_dia_raw.groupby("Nome"):
        ...
        if dff.empty:
            continue
        # Excluir 'FINAL DE EXPEDIENTE' do resumo
        dff = dff[dff["Descrição da Operação"].str.upper().str.strip() != "FINAL DE EXPEDIENTE"]
        if dff.empty:
            continue
        ...
        fim = dff["Fim"].max()
        total_min = dff["Duracao Min"].sum()
        total_horas = total_min / 60.0

        stats_min = dff.groupby("Tipo Parada")["Duracao Min"].sum()
        total_para_pct = stats_min.sum() if stats_min.sum() > 0 else 1

        def pct(tipo):
            return f"{(float(stats_min.get(tipo, 0)) / total_para_pct) * 100:.1f}%"
        ...
        linhas.append({
            ...
            "Horas Trabalhadas": f"{total_horas:.2f}h",
